...
⭐ Types of Rollout Usage in Model-Based Reinforcement Learning

Model-based RL algorithms use imagined trajectories (rollouts) in fundamentally different ways.
Below is a clean taxonomy used in modern literature—organized into background planning, decision-time planning, and hybrid methods.

⭐ I. Background Planning (Offline / Reactive Policy Search)

Rollouts are generated offline using the learned model and injected into the policy/value training process. The agent does not explicitly plan at decision time.

1. Policy Training via Dyna-Style Rollouts

Synthetic rollouts are added to the replay buffer, augmenting real experience to accelerate policy learning.

Dyna-Q: An Integrated Architecture for Learning, Planning, and Reacting
ACM, 1991 — One-step rollouts

MBPO: Model-Based Policy Optimization
ICLR, 2019 — Short multi-step rollouts with a trust-region

Algorithmic Framework for MBRL with Theoretical Guarantees
ICLR, 2019 — Multi-step rollouts for policy evaluation

2. Backpropagation Through Time (Differentiable Models)

Model derivatives are used to directly optimize policies.

PILCO: A Model-Based and Data-Efficient Approach to Policy Search
ICML, 2011 — Greedy exploration with Gaussian Processes

3. Value Expansion (Improving TD Targets)

Rollouts extend the horizon for value backups, but are not stored in the replay buffer.
...
4. Imagination-Based / Auxiliary Rollout Features

Rollouts serve as additional inputs to the policy network.

Imagination-Augmented Agents (I2A)
NeurIPS, 2017 — Policy conditions on imagined futures

⭐ II. Decision-Time Planning (Online / Lookahead Planning)

Rollouts are generated at inference time for real-time decision making.

1. Tree Search / Lookahead Planning

AlphaGo Zero — Nature, 2017

MuZero: Planning with a Learned Model — Nature, 2020

These use MCTS with a learned dynamics model for action selection.

2. Shooting Methods (Trajectory Optimization)

The agent samples action sequences, simulates them forward, and chooses the best one.

Probabilistic Dynamics Models for Data-Efficient RL (PETs family)
NeurIPS, 2018

Robust Constrained MPC
2005 — Optimization-based planning under uncertainty
